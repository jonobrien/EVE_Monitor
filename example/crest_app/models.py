...
class EveUser(AbstractUser):
    ...
    def _eve_auth(self):
        ...
        return self.social_auth.filter(provider='eveonline')[0]

    def _get_crest_tokens(self):
        """get tokens for authenticated CREST"""
        expires_in = time.mktime(
            dateutil.parser.parse(
                self._eve_auth.extra_data['expires']  # expiration time string
            ).timetuple()                             # expiration timestamp
        ) - time.time()                               # seconds until expiration
        return {
            'access_token': self._eve_auth.extra_data['access_token'],
            'refresh_token': self._eve_auth.extra_data['refresh_token'],
    ...
    def character_id(self):
        ...
        return self._eve_auth.extra_data['id']
